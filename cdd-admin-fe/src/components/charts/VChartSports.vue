<template>
  <el-card class="card">
    <div class="card-header">
      <div class="meta">
        <div class="analysis-label">今日竞技彩投注总额</div>
        <div class="analysis-total">{{ total_money | filterMoney }}</div>
      </div>
      <div class="platform">
        H5 <span>{{ h5_money | filterMoney }}</span>
        Android <span>{{ android_money | filterMoney }}</span>
        iOS <span>{{ ios_money | filterMoney }}</span>
      </div>
    </div>
    ...
    <div class="card-footer">
      今日订单数 <span>{{ order_count }}</span>
      下单人次 <span>{{ user_count }}</span>
    </div>
  </el-card>
</template>
